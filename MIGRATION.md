# Deriv API Migration Guide — v1 → v2
...
This document tracks the migration of the `analysisprofithub` project from the legacy Deriv WebSocket API to the modern **Deriv API v2** (REST + OTP-authenticated WebSocket) architecture.

---

## What Changed in Deriv API v2

### 1. WebSocket Endpoints

| Type     | Legacy (v1 / deprecated)                          | v2 (current)                                              |
|----------|---------------------------------------------------|-----------------------------------------------------------|
| Public   | `wss://ws.derivws.com/websockets/v3?app_id=…`     | `wss://ws.deriv.com/v2/options/public?app_id=…`           |
| Demo     | `wss://ws.derivws.com/websockets/v3?app_id=…`     | `wss://ws.deriv.com/v2/options/demo` _(OTP auth)_         |
| Real     | `wss://ws.derivws.com/websockets/v3?app_id=…`     | `wss://ws.deriv.com/v2/options/real` _(OTP auth)_         |

### 2. Authentication Flow

**Legacy:** Direct WebSocket `{ authorize: "<api_token>" }` over a single endpoint.
...
3. Call REST `GET /trading/v1/options/accounts` to list accounts
...
| | Legacy | v2 |
|---|---|---|
| REST Base | `https://api.deriv.com/` | `https://api.deriv.com/v2` |
...
- **AFTER:** All endpoints updated to v2:
  ```ts
...
- `WS_URL` default updated to `wss://ws.deriv.com/v2/options/public`

### Auth & OTP

#### `lib/deriv-otp-handler.ts`
- Fixed hardcoded URL double-path bug: was `https://api.deriv.com/v2/trading/v1/...`, now uses `${DERIV_API.REST_BASE}/trading/v1/...`
...
- `DERIV_API.WEBSOCKET` alias resolves to v2 public endpoint

### WebSocket Managers

#### `lib/chart-websocket-manager.ts`
- Uses `DERIV_API.WEBSOCKET` (now v2 endpoint)

#### `lib/deriv-rest-client.ts`
- All REST calls use `DERIV_API.REST_BASE` (`https://api.deriv.com/v2`)
...
- `API_URL` updated to `wss://ws.deriv.com/v2/options/public`
...
Expected output:
```
✅ Connected to wss://ws.deriv.com/v2/options/...
...
- [Deriv WebSocket API v2](https://developers.deriv.com/docs/websocket/)
